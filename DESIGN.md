...
- `dialogue-overlay`는 대사가 있는 컷씬(예: Chapter03 장치 작동 전 경고 장면)에서 배경 장면과 플레이어를 그대로 보여준 채 하단에 화자 이름과 대사를 표시한다. `feedback` 색 테두리로 강조하고, 마지막 대사 이후에는 짧은 페이드를 거쳐 다음 장면으로 전환한다.
